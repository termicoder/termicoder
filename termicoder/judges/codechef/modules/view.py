...
def contests():
    ...
    display_strings.append("|"+style.sno("SNo",3)+
                    ...
                    "|"+style.time("End",10)+"|")
    display_strings.append(contest_divison_line1())
    for sno,contest in zip(range(len(contest_list["contests"])),contest_list["contests"]):
    ...
    if(contest_list["others"]!=None):
        ...
        if(not session.is_logged_in(ensure=True)):
            display_strings.extend(["You are not logged in","There may be some more contests for which","codechef requires your login to view"])
